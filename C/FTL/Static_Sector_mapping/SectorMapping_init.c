...
{
	Flash_memory = fopen("Sector_Mapping_FM.txt", "w"); //쓰기모드로 연다.

	int nob = ((value * 1024 * 1024) / 512) / 32; // 블록 개수
	int nos = ((value * 1024 * 1024) / 512); //섹터 개수

	mtable.num_sec = nos;

	mtable.table = (int*)malloc(sizeof(int) * nos); //섹터의 개수만큼 테이블 할당
	mtable.erase_count = (int*)malloc(sizeof(int) * nos); //각 테이블마다 삭제카운트 적기위해 할당
	mtable.write_count = (int*)malloc(sizeof(int) * nos); //각 테이블마다 쓰기카운트 적기위해 할당
